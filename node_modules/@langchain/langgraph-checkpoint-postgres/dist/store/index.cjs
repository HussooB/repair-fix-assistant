...
*/
var PostgresStore = class PostgresStore extends __langchain_langgraph_checkpoint.BaseStore {
	core;
	vectorOps;
	crudOps;
	searchOps;
	...
	constructor(config) {
		...
		this.core = new require_database_core.DatabaseCore(pool, config.schema || "public", config.ttl, config.index, config.textSearchLanguage);
	...
	*/
	async runStoreMigrations() {
		...
		try {
			...
			const migrations = require_store_migrations.getStoreMigrations(migrationConfig);
			try {
